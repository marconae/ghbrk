...
/// Waits until `mock-github` answers a plain TLS request from inside
/// `devenv`. `wait_for_devenv` only confirms `devenv` itself is reachable —
/// the `mock-github` service in its own container can still be finishing
/// startup at that point, so a single one-shot curl right after can race it
/// on a cold/loaded runner. Retries a lightweight curl until it exits zero.
fn wait_for_mock_github(timeout: Duration) {
    let deadline = Instant::now() + timeout;
    while Instant::now() < deadline {
        let out = docker_exec(
            DEVENV_CONTAINER,
...
/// Allow-all `pr_open`/`pr_comment` policy: matches any user/org/repo. Used
/// by the `gh pr create`/`gh pr comment` stdin-forwarding regression tests.
fn gh_pr_allow_policy() -> &'static str {
    "rules:\n  \
     - user: \"*\"\n    \
       org: \"*\"\n    \
       repo: \"*\"\n    \
...
/// Provisions the `devenv` container for a broker `gh` run: copies the
/// static binary in, plants `policy`, and creates the credential tree for
/// `root` (the container's only user). When `with_token` is false the `token`
/// file is omitted so the broker's credential load fails.
fn provision_devenv(with_token: bool, policy: &str) {
    ...
    write_file_in_container(DEVENV_CONTAINER, "/tmp/ghbrk-policy.yaml", policy, "644");
}
...
// ---------------------------------------------------------------------------
// Stdin-forwarding regression coverage (Group D, task 8.1)
//
// Reproduces the three symptoms the plan's investigation traced to the
// missing stdin frame: `gh pr create --body-file -`, `gh pr comment
// --body-file -`, and `gh pr comment --body-file <path>` all reach the mock
// GitHub API carrying the real body text rather than an empty one. Each test
// asserts on `mock-github`'s own record of the GraphQL request it received
// (`/tmp/graphql-requests.log`), independent of whether `gh` goes on to
// report success, since that record is the one place the forwarded bytes are
// observable from outside the broker.

/// Reads the mock GitHub API's append-only GraphQL request log.
fn read_graphql_log() -> String {
    let out = docker_exec(
        "ghbrk-it-mock-github",
        &["cat", "/tmp/graphql-requests.log"],
    );
    String::from_utf8_lossy(&out.stdout).into_owned()
}

/// Runs `ghbrk gh <args>` inside `devenv`, in `cwd`, through the broker
/// socket, writing `stdin` to the child's standard input and then closing it
/// so the gateway observes end-of-file on that direction.
fn run_ghbrk_gh_in_devenv(cwd: &str, args: &[&str], stdin: &[u8]) -> std::process::Output {
    use std::process::Stdio;
    ...
    let mut child = cmd.spawn().expect("docker exec ghbrk gh");
    child
        .stdin
        .as_mut()
        .unwrap()
        .write_all(stdin)
        .expect("write stdin to ghbrk gh");
    // `wait_with_output` closes the stdin handle before waiting, giving the
    // gateway pump the end-of-file it needs to send `StdinEof`.
    child.wait_with_output().expect("wait ghbrk gh")
}

/// Creates `/tmp/repo` in `devenv` with a commit on `feature`, an `origin`
/// remote pointing at the mock host, and a remote-tracking ref for `feature`
/// already matching `HEAD`. `gh pr create` then finds the branch already
/// "pushed" and never attempts an actual `git push`, which the mock host
/// cannot serve (it answers only the GraphQL API, not the git smart-HTTP
/// protocol).
fn seed_pr_create_repo_in_devenv() {
    let _ = docker_exec(DEVENV_CONTAINER, &["rm", "-rf", "/tmp/repo"]);
    let setup = docker_exec(
        ...
        &[
            ...
            "set -e; \
             ...
             git config user.name 'ghbrk harness' && \
             echo seed > f && git add f && git commit -q -m seed && \
             git remote add origin https://mock-github/acme/web.git && \
    ...
    assert!(
        ...
        "seeding /tmp/repo for gh pr create failed: stdout={} stderr={}",
...
#[test]
fn gh_pr_comment_body_file_dash_forwards_stdin_body() {
    if skip_if_no_docker("gh_pr_comment_body_file_dash_forwards_stdin_body") {
        return;
    }
    let _lock = GLOBAL_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    let _compose = start_compose();
    ...
    start_daemon_in_devenv();
    let out = run_ghbrk_gh_in_devenv(
        "/tmp",
        &[
            "pr",
            "comment",
        ...
        b"regression body via stdin\n",
    );
    stop_daemon_in_devenv();

    assert!(
        out.status.success(),
        "gh pr comment --body-file - through broker failed: stdout={} stderr={}",
        String::from_utf8_lossy(&out.stdout),
        String::from_utf8_lossy(&out.stderr)
    );
    let log = read_graphql_log();
    assert!(
        log.contains("regression body via stdin"),
        "mock GitHub API never received the comment body: log={log}"
    );
}

#[test]
fn gh_pr_comment_body_file_path_forwards_body() {
    if skip_if_no_docker("gh_pr_comment_body_file_path_forwards_body") {
        return;
    }
    let _lock = GLOBAL_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    let _compose = start_compose();
    ...
    assert!(
        out.status.success(),
        "gh pr comment --body-file <path> through broker failed: stdout={} stderr={}",
        String::from_utf8_lossy(&out.stdout),
        String::from_utf8_lossy(&out.stderr)
    );
    let log = read_graphql_log();
    assert!(
        log.contains("regression body via file path"),
        "mock GitHub API never received the comment body: log={log}"
    );
}

#[test]
fn gh_pr_create_body_file_dash_forwards_stdin_body() {
    if skip_if_no_docker("gh_pr_create_body_file_dash_forwards_stdin_body") {
        return;
    }
    let _lock = GLOBAL_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    let _compose = start_compose();
    ...
    let out = run_ghbrk_gh_in_devenv(
        ...
        b"regression pr body via stdin\n",
    );
    stop_daemon_in_devenv();

    assert!(
        out.status.success(),
        "gh pr create --body-file - through broker failed: stdout={} stderr={}",
        String::from_utf8_lossy(&out.stdout),
        String::from_utf8_lossy(&out.stderr)
    );
    let log = read_graphql_log();
    assert!(
        log.contains("regression pr body via stdin"),
        "mock GitHub API never received the PR body: log={log}"
    );
}
