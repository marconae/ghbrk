...
/// A mock broker that accepts one connection, reads the request frame, sends
/// a single `StdoutChunk` carrying `output` followed by `Exit { code }`, then
/// closes. Returns the socket path.
fn start_mock_broker_with_check_output(output: &'static str, code: i32) -> (TempDir, PathBuf) {
    use ghbrk::protocol::ServerFrame;
    ...
    std::thread::spawn(move || {
        ...
        rt.block_on(async move {
            ...
            ghbrk::protocol::write_frame(&mut writer, &ServerFrame::Exit { code })
                .await
                .unwrap();
        });
    });
...
// ── Shared-filesystem check tests ────────────────────────────────────────────

#[test]
fn shared_filesystem_matching_tmp_identity_is_ok() {
    // A real broker running in this same test process resolves /tmp to the
    // same directory `doctor` does, so the identities match.
    let h = start_broker(deny_all_policy());
    let out = std::process::Command::new(bin())
        .arg("doctor")
        .env("GHBRK_SOCKET", &h.socket_path)
    ...
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
        stdout.contains("Shared filesystem: OK"),
        "expected 'Shared filesystem: OK' in stdout: {stdout}"
    );
}

#[test]
fn shared_filesystem_mismatched_tmp_identity_names_private_tmp() {
    // Every credential check passes and only the shared-filesystem check
    // fails, which is the shape a host with `PrivateTmp=true` and a healthy
    // credential set produces. The broker folds both into one non-zero exit
    // code, so the heading must come from the credential lines themselves:
    // naming the credential tree here would send the operator to the wrong
    // subsystem.
    let output = "SSH key: OK\nToken: OK\nGitHub API: OK (user: octocat)\n\
        Shared filesystem: ERROR (caller /tmp is dev 8:1, broker /tmp is dev 0:34; \
        caused by PrivateTmp= in /etc/systemd/system/ghbrk.service; broker /tmp mount root is \
        /systemd-private-9302792eb91a42bbbeef83f282d9bd85-ghbrk.service-ApAiqY/tmp; remove that \
        directive from /etc/systemd/system/ghbrk.service and restart the service)\n";
    let (_tmp, socket_path) = start_mock_broker_with_check_output(output, 1);
    let out = std::process::Command::new(bin())
        .arg("doctor")
        .env("GHBRK_SOCKET", &socket_path)
        .output()
    ...
    assert!(
        ...
        "a shared-filesystem-only failure must not be reported as a credential failure: {stdout}"
    );
    assert!(
        !stdout.contains("Credentials: FAILED"),
        "expected no credential failure heading when every credential line passed: {stdout}"
    );
    assert!(
        stdout.contains("Shared filesystem: ERROR"),
        "expected the ERROR line to reach stdout: {stdout}"
    );
    assert!(stdout.contains("PrivateTmp="), "{stdout}");
    assert!(
        stdout.contains("/etc/systemd/system/ghbrk.service"),
        "expected the remediation naming the unit file: {stdout}"
    );
    assert!(
        !out.status.success(),
        "a Shared filesystem ERROR must exit non-zero"
    );
}

#[test]
fn absent_filesystem_line_reports_unsupported_and_keeps_exit_status() {
    // A daemon released before this change ignores the caller_tmp identity
    // and never emits a `Shared filesystem:` line at all. Compare against an
    // otherwise-identical run that does report the line: everything but the
    // check tool's response is held constant (same policy file, same
    // environment), so a difference in exit status can only come from the
    // absent line itself.
    let tmp = tempfile::tempdir().unwrap();
    let policy_path = write_policy_file(tmp.path(), valid_policy_yaml());
    ...
    let (_tmp1, socket_with_line) = start_mock_broker_with_check_output(baseline_output, 0);
    let baseline = std::process::Command::new(bin())
        .arg("doctor")
        .env("GHBRK_SOCKET", &socket_with_line)
        .env("GHBRK_POLICY", &policy_path)
        .output()
        .expect("failed to run ghbrk doctor");

    let unsupported_output = "SSH key: OK\nToken: OK\nGitHub API: OK (user: octocat)\n";
    let (_tmp2, socket_without_line) = start_mock_broker_with_check_output(unsupported_output, 0);
    let unsupported = std::process::Command::new(bin())
        .arg("doctor")
        .env("GHBRK_SOCKET", &socket_without_line)
        .env("GHBRK_POLICY", &policy_path)
        .output()
        .expect("failed to run ghbrk doctor");

    let unsupported_stdout = String::from_utf8_lossy(&unsupported.stdout);
    assert!(
        unsupported_stdout.contains("Shared filesystem: UNSUPPORTED"),
        "expected the unsupported line when the daemon reports no Shared filesystem line: {unsupported_stdout}"
    );
    let baseline_stdout = String::from_utf8_lossy(&baseline.stdout);
    assert_eq!(
        baseline.status.code(),
        unsupported.status.code(),
        "the absent Shared filesystem line must not change the exit status: \
         baseline exit={:?} stdout={baseline_stdout:?}, \
         unsupported exit={:?} stdout={unsupported_stdout:?}",
        baseline.status.code(),
        unsupported.status.code(),
    );
}
