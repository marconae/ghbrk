...
use ghbrk::protocol::{
    read_frame, write_frame, ClientFrame, ProtocolError, ServerFrame, MAX_FRAME_LEN,
};
...
/// Verify that a client-frame header declaring a length beyond `MAX_FRAME_LEN`
/// is rejected the same way an oversized server frame is: the decoder must
/// reject on the length prefix alone, without attempting to read the body.
#[tokio::test]
async fn oversized_client_frame_returns_frame_too_large() {
    let oversize: u32 = MAX_FRAME_LEN + 1;
    ...
    let result: Result<ClientFrame, _> = read_frame(&mut reader).await;
    write_task.await.unwrap();
