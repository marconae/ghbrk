...
- **`ghbrk git` commands**: the client reads the origin remote URL from the repository's git configuration. For a plain checkout that is `.git/config` in the invoking user's working directory; for a linked worktree, the remote comes from the shared common git directory named through the working directory's `.git` file, not from a `.git` directory in the working directory.
- Discovery reads an optional `commondir` file in whichever git directory it finds, whether that is a `.git` directory or the private directory named by a `.git` file's `gitdir:` pointer. That file names the shared common directory that holds `config`; without it, the discovered directory is its own common directory.
- A linked worktree's branch comes from its own `HEAD`, not from the main checkout's `HEAD`.
- A submodule checkout resolves to its own remote, not the superproject's.
- Discovery stops at the first `.git` entry found while walking upward from the working directory: an entry with no `config` file, or one that is neither a directory nor a regular file, is never skipped in favor of an enclosing repository.
