...
async fn process_request(
    ...
    if request.tool == Tool::Check {
        ...
        let inputs = crate::health_check::CheckInputs {
            creds_root: &creds_root,
            user: username,
            caller_tmp: request.caller_tmp,
        };
    ...
    stream_child_over_connection(&spec, &request, stream).await;
    Ok(())
}
...
async fn stream_child_over_connection(
    ...
    let relayed = if request.client_frames {
        let (client_frames, mut responses) = stream.split();
        stream_child(spec, client_frames, &mut responses).await
    } else {
        stream_child(spec, tokio::io::empty(), stream).await
    };
...
async fn handle_check_request(
    stream: &mut UnixStream,
    inputs: crate::health_check::CheckInputs<'_>,
) -> Result<(), ConnectionError> {
    let creds_root = inputs.creds_root;
    let username = inputs.user;

    let mut output: Vec<u8> = Vec::new();
    let all_ok = crate::health_check::run_checks(inputs, &mut output);
...
async fn build_env(
    ...
    match tool {
        Tool::Gh => Ok((gh_env(&creds.token), None, None)),
